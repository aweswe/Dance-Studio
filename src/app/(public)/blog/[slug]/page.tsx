      ...
      <article className="max-w-4xl mx-auto px-6 md:px-16 py-16 md:py-24">
        ...
        <header className="mb-12 text-center">
          <div className="flex items-center justify-center gap-4 text-[10px] font-bold tracking-[2px] uppercase mb-6">
            <span className="text-bl-ink">{post.author?.name || 'Academy'}</span>
            <span className="w-1 h-1 rounded-full bg-line-strong" />
            <span className="text-ink-2">{post.published_at ? formatDate(post.published_at) : 'Draft'}</span>
          </div>
        ...
        <Reveal y={20}>
          <div className="prose prose-lg md:prose-xl prose-neutral dark:prose-invert max-w-none prose-headings:font-display prose-headings:font-normal prose-a:text-bl prose-img:rounded-xl">
            {post.content ? (
              ...
              <p className="text-ink-2 text-center italic py-10">Content is empty.</p>
            )}
          </div>
        </Reveal>

        {/* Footer / Author */}
        <footer className="mt-20 pt-10 border-t border-line-strong">
          <div className="flex items-center gap-6 bg-canvas-muted p-8 rounded-card">
            ...
            <div>
              <h3 className="text-xs font-bold tracking-[2px] uppercase mb-2">Written by {post.author?.name || 'Academy'}</h3>
              {post.author?.bio && (
          ...
          <div className="mt-10 text-center">
            <Link
              href="/blog"
              className="inline-block text-[11px] font-semibold tracking-[2px] uppercase py-4 px-8 border border-line-strong hover:border-bl-ink hover:text-bl-ink transition-all focus-visible:focus-ring active:scale-[0.98]"
            >
              &larr; Back to Blog
            </Link>
          </div>
