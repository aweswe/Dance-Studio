    ...
    <div className="space-y-6">
      <Card className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          ...
          <div>
            <label className="block text-sm text-mu mb-1">Date</label>
            <Input
              type="date"
              value={date}
      ...
      {report && !report.success && (
        <Card className="p-6 text-sm text-red-500">{report.error}</Card>
      )}
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card className="p-6 text-center">
              ...
              <p className="text-xs font-display tracking-[2px] text-mu uppercase mt-1">Present</p>
            </Card>
            <Card className="p-6 text-center">
              <p className="font-display text-3xl text-red-500">{report.counts.absent}</p>
              <p className="text-xs font-display tracking-[2px] text-mu uppercase mt-1">Absent</p>
            </Card>
            <Card className="p-6 text-center">
              <p className="font-display text-3xl text-gold">{report.counts.leave}</p>
              <p className="text-xs font-display tracking-[2px] text-mu uppercase mt-1">On Leave</p>
            </Card>
            <Card className="p-6 text-center">
              <p className="font-display text-3xl text-mu">{report.unmarked?.length ?? 0}</p>
              <p className="text-xs font-display tracking-[2px] text-mu uppercase mt-1">Unmarked</p>
            </Card>
          </div>

          <Card className="p-6">
            <h3 className="font-display text-xl text-blk mb-4 flex items-center gap-2">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-light border-b border-gray-200">
                    <th className="px-4 py-3 text-xs font-display tracking-[2px] text-mu uppercase">Student</th>
                    <th className="px-4 py-3 text-xs font-display tracking-[2px] text-mu uppercase">ID</th>
                    <th className="px-4 py-3 text-xs font-display tracking-[2px] text-mu uppercase">Status</th>
                    <th className="px-4 py-3 text-xs font-display tracking-[2px] text-mu uppercase">Marked</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {(report.roster ?? []).map((s) => {
                    ...
                    return (
                      <tr key={s.id}>
                        <td className="px-4 py-3 font-medium text-blk">{s.name}</td>
                        <td className="px-4 py-3 text-sm text-mu">{s.student_id_display}</td>
                        ...
                        <td className="px-4 py-3">
                          {mark ? statusBadge(mark) : <span className="text-sm text-mu italic">Not marked</span>}
                        </td>
                      </tr>
                    )
                  })}
                  {(report.roster ?? []).length === 0 && (
                    <tr>
                      <td colSpan={4} className="px-4 py-8 text-center text-mu">
